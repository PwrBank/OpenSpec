---
name: code-review
description: Reviews code for security vulnerabilities, bugs, performance issues, and adherence to project patterns during archive review. Invoked automatically by the archive process (unless --skip-review flag). Reviews implementation against proposal.md, tasks.md, and spec deltas to ensure quality and consistency.
...
- proposal.md, tasks.md, and spec deltas for context
...
1. **Get Changes**
   ```bash
   git diff main..HEAD  # Get all changes on feature branch
   ```

2. **Review Context**
   - Read openspec/changes/[change-id]/proposal.md - Understand intent
   - Read openspec/changes/[change-id]/tasks.md - Know what should be done
   - Read openspec/changes/[change-id]/specs/*/spec.md - See spec deltas
...
6. **Review Focus**
   ...
   - Does it implement what was proposed?
...
**Scope Issues:**
- Implementation doesn't match proposal.md
- Tasks in tasks.md not completed
- Spec deltas not implemented
...
- Spec deltas implemented as proposed
...
- Does it match the approved proposal?
